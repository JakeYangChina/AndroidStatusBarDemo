...
class CoreStatusBarLollipop {
    private static ValueAnimator sAnimator;

    static void setStatusBarColor(Activity activity, int statusColor) {
        Window window = activity.getWindow();
        //有的手机是半透明渐变，所以先清除背景半透明，再采用绘制背景，防止部分手机状态栏渐变，小米手机6.0需要单独兼容
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);

        //设置状态栏半透明，设置颜色失效，位置延伸到顶部
        ...
        if (mChildView != null) {
            ...
            //重新计算布局大小，View.requestFitSystemWindows()
    ...
    static void translucentStatusBar(Activity activity, boolean statusBarIsTransparent) {
        ...
        if (statusBarIsTransparent) {
            ...
            //设置View.SYSTEM_UI_FLAG_LAYOUT_STABLE显示状态栏，同时全屏显示View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN（全屏显示，但状态栏不会被隐藏，状态栏依然可见，布局延伸到顶部）
